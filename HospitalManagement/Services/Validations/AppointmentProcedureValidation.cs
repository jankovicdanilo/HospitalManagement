...
namespace HospitalManagement.Services.Validations
{
    public class AppointmentProcedureValidation : IAppointmentProcedureValidation
    {
        ...
        public AppointmentProcedureValidation(IAppointmentRepository appointmentRepository, IProcedureRepository procedureRepository, 
        ...
        public async Task<Result> ValidateForDelete(int appointmentId, int procedureId)
        {
            ...
            if (link == null)
            {
                return Result.Fail($"Procedure {procedureId} is not linked to appointment {appointmentId}", "PROCEDURE_NOT_LINKED");
            }
